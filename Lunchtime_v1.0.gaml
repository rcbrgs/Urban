...
global {
	...
	float step <- 1 #mn;
	date starting_date <- date("2017-07-10") add_hours 4;
...
species people skills:[moving] {
	...
	reflex time_to_lunch when: current_date.hour = start_lunch_time and objective = "working"
	{
		objective <- "eating" ;
		the_target <- any_location_in (lunch_place);
	}

	reflex time_to_work_afternoon when: current_date.hour = end_lunch_time and objective = "eating"
	{
		objective <- "working" ;
		the_target <- any_location_in (working_place);
	}
