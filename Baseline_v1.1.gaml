...
global {
	...
	float step <- 30 #mn;
	date starting_date <- date("2016-12-31") add_hours 4;
	date ending_date <- date("2018-01-01") ;
	int nb_people <- 20000;
	...
	init {
		...
		create people number: nb_people {
			...
			start_lunch_time <- rnd(min_lunch_time, min_lunch_time + 2);
			end_lunch_time <- start_lunch_time + 1;
...
species people skills:[moving] {
	...
	reflex time_to_eat when: current_date.hour != hourArrived and objective = "resting" {
		objective <- "eating" ;
		the_target <- any_location_in (lunch_place);
		hourArrived <- current_date.hour ;
	}

	reflex time_to_rest when: current_date.hour != hourArrived and objective = "eating" {
		objective <- "resting" ;
